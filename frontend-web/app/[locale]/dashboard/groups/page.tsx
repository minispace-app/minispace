    ...
    <div className="flex h-full overflow-hidden">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white/80 backdrop-blur-sm shadow-soft flex-shrink-0 my-3 ml-3 rounded-xl overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto py-2 px-2">
          {groups.length === 0 && (
            <p className="px-4 py-3 text-body text-ink-muted">{t("noGroups")}</p>
          )}
          {groups.map((group) => {
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{ backgroundColor: group.color || "#6B7CFF" }}
                />
      ...
      <div className="md:hidden flex flex-col h-full w-full overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 flex-shrink-0">
          ...
          {canWrite && (
            <button
              onClick={() => setShowForm(true)}
              className="flex items-center gap-1 px-3 py-1.5 bg-ink text-white rounded-pill text-body hover:opacity-90 transition-all duration-[180ms]"
            >
            ...
            <form onSubmit={handleCreate} className="p-5 space-y-3">
              ...
              <div className="flex gap-3 pt-2">
                <button
                  type="submit"
                  disabled={saving}
                  className="px-5 py-2 bg-ink text-white rounded-pill text-body hover:opacity-90 transition-all duration-[180ms] disabled:opacity-50"
                >
